...
function processLine(line: string, name: string): string | undefined {
  ...
  const fixedLine = fixTables(fixMarkdown(line));

  const homeLinkMatch = fixedLine.match(/\[Home\]\(.\/index\.md\) &gt; (.*)/);
  if (homeLinkMatch && homeLinkMatch[0]) {
    return name !== "rlog" ? homeLinkMatch[1] : undefined;
  }

  return fixedLine;
}

async function processFile(rootDir: string, outputDir: string, file: string): Promise<void> {
