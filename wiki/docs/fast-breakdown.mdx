...
`rLog` provides a default ("global") instance that you can use, but the intended usage is to create individual instances
per file or flow.
...
**rLog** has a moderate number of configurable options available via the [RLogConfig](#) and [SerializationConfig](#)
types.
...
This can be a huge help in avoiding extra verbose logs until you actually need them (ie; when debugging).
...
If you have any pending questions or want to learn more, you can read through our [API reference](#), which includes
